...
struct NaiveDFT_omp {
    static constexpr char Name[] = "NaiveDFT omp";
    const std::size_t N;

    NaiveDFT_omp(std::size_t N) : N(N) {
...
struct DitFFT_omp {
    ...
    DitFFT_omp(std::size_t N) : N(N), L(__builtin_ctzll(N)), W(N), I(N) {
        using namespace std;
        auto w = _2Pi / (Real) N;
#pragma omp parallel for
        for (size_t i = 0; i < N; ++i) {
...
// For convolution use, no bit-reverse-copy performed, performed in-place
// N must be power of 2
struct DifDitFFT_omp {
    static constexpr char Name[] = "DifDitFFTomp";
    const std::size_t N;
    const int L;
    std::vector<Comp> W;

    DifDitFFT_omp(std::size_t N) : N(N), L(__builtin_ctzll(N)), W(N) {
        using namespace std;
        auto w = _2Pi / (Real) N;
#pragma omp parallel for
        for (size_t i = 0; i < N; ++i)
...
// Arbitrary N
struct BluesteinFFT {
    static constexpr char Name[] = "Bluestein";
    const std::size_t N;
    const bool pow2;
...
struct BluesteinFFT_omp {
    static constexpr char Name[] = "Bluesteinomp";
    const std::size_t N;
    const bool pow2;
    std::vector<Comp> A, B, C;
    union {
        char c;
        DitFFT_omp dit;
        DifDitFFT_omp difdit;
    };

    BluesteinFFT_omp(std::size_t N) : N(N), pow2(isPow2(N)) {
        if (pow2) {
            new(&dit) DitFFT(N);
            return;
        }
...
#pragma omp parallel for
        for (size_t i = 0; i < N; ++i)
            A[i] = X[i] * conj(C[i]);
#pragma omp parallel for
        for (size_t i = (ptrdiff_t) N; i < difdit.N; i++) {
            A[i] = Comp{};
        }
//        fill(A.begin() + (ptrdiff_t) N, A.end(), Comp{});
        difdit.dft(A.data());
...
void testIdentity(Tr &&T) {
    ...
    cout << "[" << T.Name << "] \terr(x, idft(dft(x))) = " << err << "\t\t\tRuntime " << tick << endl;
}
...
#pragma omp parallel for
    ...
    cout << "[" << T.Name << "] \terr(a*b, idft(dft(a)*dft(b))) = " << err << "\t\tRuntime " << tick << endl;
}
...
void testBasic() {
    ...
    NaiveDFT naive(N);
    NaiveDFT_omp naiveomp(N);
//    DitFFT dit(N);
//    DifFFT dif(N);
    DifDitFFT difdit(N);
    DifDitFFT_omp difDitFftOmp(N);
//    testAll(naive);
    testAll(naiveomp);
//    testAll(dit);
//    testAll(dif);
//    testCmp(naive, dit, dif);
    testDifDit(difdit);
    testDifDit(difDitFftOmp);
}

void testArbitrary() {
    using namespace std;
    constexpr size_t N = 1024*113;
    NaiveDFT naive(N);
    BluesteinFFT bluestein(N);
    BluesteinFFT_omp bluesteinFftOmp(N);
    testAll(bluestein);
    testAll(bluesteinFftOmp);
